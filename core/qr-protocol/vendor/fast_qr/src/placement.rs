...
///
/// **AirFerry modification**: when `mask` is `Some(m)` (caller-specified), the
/// 8-mask evaluation loop is skipped entirely — the mask is applied directly.
/// This is a ~10× speedup because the loop clones the full `[Module; 31329]`
/// matrix 8 times and scores each copy. Any valid mask is decodable by ZXing
/// (it reads the mask ID from format info and inverts), so the optimal-mask
/// search is unnecessary for AirFerry's screen→camera use case.
pub fn place_on_matrix(
    ...
    // Fast path: caller specified a fixed mask — skip the 8-mask search loop.
    if let Some(fixed_mask) = *mask {
        default::create_matrix_format_info(&mut qr, quality, fixed_mask);
        datamasking::mask(&mut qr, fixed_mask);
        qr.mask = Some(fixed_mask);
        return qr;
    }

    // Default path: evaluate all 8 masks and pick the best (lowest penalty).
